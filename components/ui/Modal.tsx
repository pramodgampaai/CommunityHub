...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 dark:bg-black/60 z-50 flex justify-center items-center p-4 modal-backdrop" onClick={onClose}>
      <div className="bg-[var(--card-bg-light)] dark:bg-[var(--card-bg-dark)] rounded-lg shadow-xl w-full max-w-md transform transition-all modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-center p-6 border-b border-[var(--border-light)] dark:border-[var(--border-dark)]">
